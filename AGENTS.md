...
本项目长期由多个代理并行推进，**共用同一个工作目录**。因此：

- **暂存必须指名路径**：`git add <path>`，**禁止 `git add -A` / `git add .`**。一次空白暂存曾把另一个工单的半成品一起提交，让一个失败的测试落进不相关的提交里、连续两个提交 CI 变红。
- **提交前先看 `git status`**：确认自己改的只有自己负责的那些文件。
- **文件所有权先划线**：并行前把共享文件（`contract/src/lib.rs`、`server/src/routes.rs` 这类只有一两行接线的地方）明确归给**其中一方**，另一方只报告需要的改动；或由协调者先把接缝留好。收窄所有权时要留意别把某方逼进"要么违规、要么设计将就"的死角。
- **交付前自己跑完整 CI 并贴结果**，并且**逐一看每个工作流**，不要只看自己以为相关的那一个。"我触发了 CI"和"我的新东西真的跑过了"是两件事：新任务若只存在于未推送的工作树里，`workflow_dispatch` 跑到的是旧版本。
- **新断言必须证明它会失败**：安全/权限/漂移/回滚这类断言，先制造一个应当被拦住的输入，确认它真的变红，再恢复。**不会失败的断言不算断言。**
